...
class Starmap{
    ...
    //private constellationColor:string = "red";
    ...
    constructor(j$:any, containerId:string, coeff:number, parameters:any){
        ...
        this.id = "sm_"+Math.round(Math.random()*1000);

        this.bgcolor = parameters.backgroundColor;
        this.starColor = parameters.starColor;

        this.constellationColor = parameters.constellationColor; // TODO проверить что приходит в конструкторе
    ...
    public destroy():void{
        console.log("Starmap ",this.id,"destroy");
    }
    ...
    public setCoord(coord:any):void{
       // console.log("set coord ",coord);
        ...
        if(!this.currentDate){
            console.error("current date undefined");
            return;
        }
        var n:any = Date.parse( this.currentDate );

        var d:any = new Date( n );
        this.now.setDate( d );

        var lon:number = this.currentCoord[0];
        var lat:number = this.currentCoord[1];
    ...
    private draw_sky( context, w, h ):void{

        // TODO если нет текущей даты, выше эмулировать ее, впоследствии переписать
    ...
    private init_stars( collection ):void{
        var clut:any = [
            "#AEC1FF",  /* bv = -0.4 */
            "#C5D3FF",
            "#EAEDFF",
    ...
    private init_planets( collection ):void {
    ...
    private draw_star( context, s ):void{
    ...
    private draw_line( context, s1, s2 ):void {
    ...
    private draw_planet( context, p ):void
    {
        p.color = "yellow";
        this.draw_star( context, p );
    }
    
    private setDateNow():void{
        var d = Date(Date.now());
