 ...
 */
var ver = "0.0.3";
console.log("Wickedpicker ver="+ver);
(function ($, window, document) {
    ...
    var instance;
    var now = new Date();
    var savedTime = [now.getHours(),now.getMinutes()];
    EventBus.addEventListener("TIME_CHANGED", (data)=>onTimeChanged(data));
    
    function onTimeChanged(data){
        savedTime = [parseInt(data.hours), parseInt(data.minutes)];
    }
    ...
    $.extend(Wickedpicker.prototype, {
        ...
        showPicker: function (element) {
            ...
            //console.log("element text:",this.getText(element), "current time:",this.getTime(), "saved time:",savedTime);

            if (this.getText(element) !== this.getTime()) {

                if(savedTime){
                    var newTime = {};
                    newTime.hours = savedTime[0];
                    newTime.minutes = savedTime[1];
                    //console.log("new time: ",newTime);
                    this.setTime(newTime);
                    this.element.val(newTime.hours+":"+newTime.minutes);
                    //console.log("input new value: ",this.element.val());
                }
                else{
                    // Check meridiem
        ...
        setTime: function (time) {
            ...
            savedTime = [time.hours, time.minutes]
        },
        ...
        changeValue: function (operator, input, clicked) {
            ...
            if (targetClass.endsWith('hours')) {
                var hours = eval(this.getHours() + operator + 1);
                this.setHours(hours);
                savedTime = [this.selectedHour, this.selectedMin];
            } else if (targetClass.endsWith('minutes')) {
                var minutes = eval(this.getMinutes() + operator + this.options.minutesInterval);
        ...
        setText: function (input) {
            //console.log("setText to input this.selectedHour="+this.selectedHour+" this.selectedMin="+this.selectedMin);
            //console.log("savedTime:",savedTime);
            //$(input).val(this.formatTime(this.selectedHour, this.selectedMin, this.selectedMeridiem, this.selectedSec)).change();
            $(input).val(this.formatTime(savedTime[0], savedTime[1], ":", "")).change();
        },
        ...
        if (!$.isFunction(Wickedpicker.prototype['_' + options])) {
            return this.each(function () {
                if (!$.data(this, "plugin_" + pluginName)) {
                    instance = new Wickedpicker(this, options);
